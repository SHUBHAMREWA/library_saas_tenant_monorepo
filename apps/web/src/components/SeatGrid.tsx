...
import { Armchair, Check, Wrench, Ban, UserCheck, X, Building2, Plus, Trash2, Phone, User, ArrowRight } from 'lucide-react';
...
interface SeatGridProps {
  ...
  onViewStudentProfile?: (studentId: string, studentName?: string) => void;
  onAddRoom?: () => void;
  onAddRow?: () => void;
  onAddSeatsToRow?: (rowName: string, roomId?: string) => void;
  onDeleteSeat?: (seatId: string) => void;
  onDeleteRow?: (rowName: string, roomId?: string) => void;
}
...
export const SeatGrid: React.FC<SeatGridProps> = ({
      ...
      <>
        <div className="flex items-center justify-between w-full gap-0.5">
          ...
          {isShared ? (
            ...
            <span className="text-[8px] font-bold px-1 py-0.2 rounded bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300">
              ☀️ 24h
            </span>
          ) : morningOccupant ? (
            <span className="text-[8px] font-bold px-1 py-0.2 rounded bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300">
              🌅 AM
            </span>
          ) : eveningOccupant ? (
            <span className="text-[8px] font-bold px-1 py-0.2 rounded bg-indigo-100 dark:bg-indigo-950 text-indigo-800 dark:text-indigo-300">
              🌇 PM
            </span>
          ) : null}
        </div>

        <div className="w-full truncate text-[10px] text-slate-700 dark:text-slate-200 font-semibold mt-1">
          {seat.status === 'AVAILABLE' ? (
            <span className="text-emerald-700 dark:text-emerald-400 font-medium">Free</span>
          ) : isShared ? (
            <span className="text-indigo-800 dark:text-indigo-300 font-bold truncate block">
              {morningOccupant?.studentName?.split(' ')[0]} + {eveningOccupant?.studentName?.split(' ')[0]}
            </span>
          ) : fullDayOccupant ? (
            <span className="truncate block font-bold text-indigo-950 dark:text-indigo-200">
              {fullDayOccupant.studentName}
            </span>
          ) : morningOccupant ? (
            <span className="truncate block font-bold text-amber-900 dark:text-amber-200">
              {morningOccupant.studentName}
            </span>
          ) : eveningOccupant ? (
            <span className="truncate block font-bold text-indigo-900 dark:text-indigo-200">
              {eveningOccupant.studentName}
            </span>
          ) : (
            <span className="truncate block">{seat.studentName || badge.label}</span>
          )}
        </div>

        {seat.shift && seat.status !== 'AVAILABLE' && !isShared && (
          <span className="text-[9px] text-slate-500 dark:text-[#a8a8a8] mt-0.5">
            {formatShift(seat.shift)}
          </span>
        )}
      </>
    );
  ...
  // Helper to render a single row card
  const renderRowCard = (rName: string, rowSeats: VisualSeatItem[], targetRoomId?: string) => {
    if (rowSeats.length === 0) return null;
    const isRowLocker = rowSeats.some((s) => s.hasLocker);
    const effectiveRoomId = targetRoomId || rowSeats[0]?.roomId || undefined;

    return (
      <div key={`${effectiveRoomId || 'default'}_${rName}`} className="space-y-2 p-3 bg-slate-50/70 dark:bg-[#121212] rounded-xl border border-slate-200 dark:border-[#262626]">
        <div className="flex items-center justify-between pb-1.5 border-b border-slate-100 dark:border-[#262626]">
          ...
          <div className="flex items-center gap-1.5">
            {onAddSeatsToRow && (
              <button
                type="button"
                onClick={() => onAddSeatsToRow(rName, effectiveRoomId)}
              ...
              <button
                ...
                onClick={() => {
                  if (window.confirm(`Are you sure you want to delete row "${rName}" and all its ${rowSeats.length} seats?`)) {
                    onDeleteRow(rName, effectiveRoomId);
                  }
                }}
  ...
  // Determine rendering structure:
  // 1. If a specific room is selected (or only 1 room exists):
  // 2. If 'All Rooms' is active and multiple rooms exist: Group by Room, then by Row!
  const hasMultipleRooms = rooms.length > 1;
  const isAllRoomsView = !selectedRoomId && hasMultipleRooms;

  // Compute room-separated groupings if in All Rooms view
  const roomSections = isAllRoomsView
    ? rooms.map((rm) => {
        const rmSeats = sortedSeats.filter(
          (s) => s.roomId === rm.id || (!s.roomId && rm.id === rooms[0]?.id)
        );
        const rmFilteredSeats = filteredSeats.filter(
          (s) => s.roomId === rm.id || (!s.roomId && rm.id === rooms[0]?.id)
        );
        const rmUniqueRows = Array.from(
          new Set([...(rm.rows || []), ...rmSeats.map((s) => s.rowName || 'Row A')])
        ).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));

        return {
          room: rm,
          allSeats: rmSeats,
          filteredSeats: rmFilteredSeats,
          rows: rmUniqueRows,
        };
      })
    : [];

  // Single room or fallback rows
  const singleViewSeats = selectedRoomId
    ? sortedSeats.filter((s) => s.roomId === selectedRoomId || (!s.roomId && selectedRoomId === rooms[0]?.id))
    : sortedSeats;
  const singleViewFilteredSeats = selectedRoomId
    ? filteredSeats.filter((s) => s.roomId === selectedRoomId || (!s.roomId && selectedRoomId === rooms[0]?.id))
    : filteredSeats;
  const singleViewRoom = rooms.find((r) => r.id === selectedRoomId) || rooms[0] || null;
  const singleUniqueRows = Array.from(
    new Set([
      ...(singleViewRoom?.rows || []),
      ...singleViewSeats.map((s) => s.rowName || 'Row A'),
    ])
  ).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
  ...
  return (
    <div className="space-y-3">
      ...
      {/* STATE A: Multiple Rooms View ("All Rooms" Tab View) */}
      {isAllRoomsView ? (
        <div className="space-y-6">
          {roomSections.map(({ room, allSeats, filteredSeats: rmFilteredSeats, rows: rmRows }) => (
            <div
              key={room.id}
              className="space-y-3 p-3.5 bg-slate-50/50 dark:bg-[#151515] rounded-2xl border border-slate-200/80 dark:border-[#262626]"
            >
              {/* Room Section Header */}
              <div className="flex items-center justify-between pb-2 border-b border-slate-200/60 dark:border-[#262626]">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/50">
                        {allSeats.length} {allSeats.length === 1 ? 'seat' : 'seats'}
                      </span>
                    </div>
                    <p className="text-[10px] text-slate-500 dark:text-neutral-400">
                      {allSeats.filter((s) => s.status === 'AVAILABLE').length} available • {allSeats.filter((s) => s.status === 'OCCUPIED').length} occupied
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  {onAddSeatsToRow && (
                    <button
                      type="button"
                      onClick={() => onAddSeatsToRow(rmRows[0] || 'Row A', room.id)}
                      className="text-xs bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-bold px-2.5 py-1.5 rounded-lg flex items-center gap-1 transition-colors cursor-pointer shadow-xs"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      <span className="hidden sm:inline">Add Seats to {room.name}</span>
              ...
              {/* Rows within this Room */}
              {rmRows.length > 0 ? (
                <div className="space-y-3">
                  {rmRows.map((rName) => {
                    const rowSeats = rmFilteredSeats
                    ...
                    return renderRowCard(rName, rowSeats, room.id);
                  })}
                </div>
              ) : (
                <div className="p-4 text-center text-xs text-slate-400 dark:text-neutral-500 bg-white dark:bg-[#121212] rounded-xl border border-dashed border-slate-200 dark:border-[#262626]">
                  No seats configured in this room yet.
                </div>
              )}
            </div>
          ))}
        </div>
      ) : singleUniqueRows.length > 0 ? (
        /* STATE B: Single Room View (or Specific Room Selected) */
        <div className="space-y-4">
          {singleUniqueRows.map((rName) => {
            const rowSeats = singleViewFilteredSeats
              .filter((s) => (s.rowName || 'Row A') === rName)
              .sort((a, b) =>
                a.seatNumber.localeCompare(b.seatNumber, undefined, {
                  numeric: true,
            ...
            return renderRowCard(rName, rowSeats, selectedRoomId || rooms[0]?.id || undefined);
          })}
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs overflow-y-auto p-3 sm:p-6 flex min-h-full items-center justify-center">
          <div className="bg-white dark:bg-[#121212] w-full max-w-md rounded-2xl p-5 sm:p-6 shadow-2xl space-y-4 my-auto relative animate-in fade-in zoom-in-95 duration-200 border border-slate-100 dark:border-[#262626]">
            ...
            {(() => {
              ...
              const fullDayOccupant =
                occupants.find((o) => (o.shift || '').toUpperCase() === 'FULL_DAY') ||
                (selectedSeat.studentName && (!selectedSeat.shift || selectedSeat.shift === 'FULL_DAY')
                  ? {
                      studentId: selectedSeat.studentId || '',
                      studentName: selectedSeat.studentName,
                      phone: selectedSeat.studentPhone || undefined,
                      photoUrl: selectedSeat.studentPhotoUrl || undefined,
                      shift: 'FULL_DAY',
                    }
                  : null);
              const morningOccupant =
                occupants.find((o) => {
                  const s = (o.shift || '').toUpperCase();
                  return s === 'MORNING' || s === 'FOUR_HOURS' || s === 'HALF_DAY';
                }) ||
                (selectedSeat.studentName &&
                (selectedSeat.shift === 'MORNING' || selectedSeat.shift === 'FOUR_HOURS' || selectedSeat.shift === 'HALF_DAY')
                  ? {
                      studentId: selectedSeat.studentId || '',
                      studentName: selectedSeat.studentName,
                      phone: selectedSeat.studentPhone || undefined,
                  ...
                  : null);
              const eveningOccupant =
                occupants.find((o) => (o.shift || '').toUpperCase() === 'EVENING') ||
                (selectedSeat.studentName && selectedSeat.shift === 'EVENING'
                  ? {
                      studentId: selectedSeat.studentId || '',
                      studentName: selectedSeat.studentName,
                      phone: selectedSeat.studentPhone || undefined,
              ...
              const renderOccupantCard = (occupant: {
                studentId?: string;
                studentName: string;
                phone?: string;
                photoUrl?: string;
                ...
                <div
                  ...
                  onClick={() => {
                    if (onViewStudentProfile) {
                      onViewStudentProfile(occupant.studentId || '', occupant.studentName);
                      setSelectedSeat(null);
                    }
                  }}
                  className="mt-2 p-2.5 rounded-xl bg-white dark:bg-[#181818] border border-slate-200/90 dark:border-[#2a2a2a] hover:border-indigo-400 dark:hover:border-indigo-500 hover:shadow-xs flex items-center justify-between gap-2.5 transition-all cursor-pointer group active:scale-[0.99]"
                  title="Click to view student profile and details"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    {occupant.photoUrl ? (
                      <img
                        src={occupant.photoUrl}
                        alt={occupant.studentName}
                        className="w-9 h-9 rounded-full object-cover border border-slate-200 dark:border-neutral-700 shrink-0 group-hover:scale-105 transition-transform"
                      />
                    ) : (
                      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 text-white font-black text-xs flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform shadow-2xs">
                        {occupant.studentName.slice(0, 2).toUpperCase()}
                      </div>
                    )}
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 px-1.5 py-0.2 rounded border border-indigo-200/60 dark:border-indigo-800/40 shrink-0">
                          View Profile
                        </span>
                      </div>
                      {occupant.phone && (
                        <span className="text-[11px] text-slate-500 dark:text-neutral-400 flex items-center gap-1 mt-0.5 truncate">
                          <Phone className="w-2.5 h-2.5 text-slate-400 dark:text-neutral-500 shrink-0" />
                  ...
                  <div className="flex items-center gap-1 shrink-0 text-indigo-600 dark:text-indigo-400 text-xs font-bold bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 px-2 py-1 rounded-lg border border-indigo-200/70 dark:border-indigo-800/50 group-hover:translate-x-0.5 transition-all">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {fullDayOccupant ? (
                        renderOccupantCard(fullDayOccupant)
                      ) : morningOccupant || eveningOccupant ? (
                        <span className="text-slate-400 dark:text-neutral-500 font-normal italic text-[11px] block mt-1.5">
                    ...
                    >
                      ...
                      {morningOccupant ? (
                        renderOccupantCard(morningOccupant)
                      ) : fullDayOccupant ? (
                        <span className="text-slate-400 dark:text-neutral-500 font-normal italic text-[11px] block mt-1.5">
                    ...
                    >
                      ...
                      {eveningOccupant ? (
                        renderOccupantCard(eveningOccupant)
                      ) : fullDayOccupant ? (
                        <span className="text-slate-400 dark:text-neutral-500 font-normal italic text-[11px] block mt-1.5">
                          Slot blocked by {fullDayOccupant.studentName} (Full Day)
                        </span>
